    ...
    <div className='flex flex-col items-center px-6 md:px-16 lg:px-24 xl:px-32 pt-20 pb-30'>
      <div className='flex flex-col md:flex-row items-center justify-between w-full'>
        <div className="flex flex-col">
          <Title align='left' title='Exclusive Offers' subTitle={subTitleText} />
          {/* Fallback subtitle rendering if Title does not display it */}
          <span className="mt-2 text-gray-500 text-sm md:max-w-lg">
            {subTitleText}
          </span>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12'>
        {exclusiveOffers.map((item) => (
            <div key={item.id} className='group relative flex flex-col items-start justify-center gap-1 pt-12 md:pt-18 px-4 rounded-x1 text-white bg-no-repeat bg-cover bg-center'
